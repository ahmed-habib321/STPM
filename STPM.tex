...
Consider the following climate change system\\
(Archana C. \& Twinkle 2022)\\
\begin{equation}
    \label{archanaSys}
    \begin{cases}
         \displaystyle \pdv{mx}{t}+m\pdv{m}{x} - fP + g\pdv{\eta}{x} =0\\
         \\
         \displaystyle \pdv{P}{t} + m\pdv{P}{x} + fm=0\\
         \\
         \displaystyle \pdv{\eta}{t} + m\pdv{\eta}{x} + \eta \pdv{\eta}{x} =0
     \end{cases}
 \end{equation}
 Let 
 \begin{align*}
    &m(x,t) = \sum_{i=0}^\infty m_i(x,t) \qquad P(x,t) = \sum_{i=0}^\infty P_i(x,t)\\
    &\eta(x,t) = \sum_{i=0}^\infty \eta_i(x,t)\\
    &m\pdv{m}{x} = \sum_{n=0}^\infty A_n(x,t) \qquad m\pdv{p}{x} = \sum_{n=0}^\infty B_n(x,t)\\
    &m\pdv{\eta}{x}+\eta\pdv{\eta}{x} = \sum_{n=0}^\infty C_n(x,t)
 \end{align*}
 Where 
 \begin{align*}
    A_n&= \frac{1}{n!} \dv[n]{}{\lambda}\left( \sum_{j=0}^n \lambda^j m_j \sum_{j=0}^n \lambda^j \pdv{m_j}{x}\right)\Bigg|_{\lambda=0} \\
    B_n&= \frac{1}{n!} \dv[n]{}{\lambda}\left( \sum_{j=0}^n \lambda^j m_j \sum_{j=0}^n \lambda^j \pdv{P_j}{x}\right)\Bigg|_{\lambda=0} \\
    C_n&= \frac{1}{n!} \dv[n]{}{\lambda}\left( \sum_{j=0}^n \lambda^j m_j \sum_{j=0}^n \lambda^j \pdv{\eta_j}{x}+ \sum_{j=0}^n \lambda^j \eta_j \sum_{j=0}^n \lambda^j \pdv{\eta_j}{x}\right)\Bigg|_{\lambda=0}
\end{align*}
Now, Integrate $(\ref{archanaSys})$ w.r.t t we get
\begin{align*}
    m(x,t)&= m(x,0) - \int_0^t m\pdv{m(x,s)}{x} ds + f \int_0^t P(x,s) ds - g\int_0^t \pdv{\eta(x,s)}{x} ds\\
    P(x,t)&= P(x,0) - \int_0^t m\pdv{P(x,s)}{x} ds - f \int_0^t m(x,s)ds\\
    \eta(x,s) = \eta(x,0) - \int_0^t m \pdv{\eta(x,s)}{x} ds - \int_0^t \eta \pdv{\eta(x,s)}{x} ds
\end{align*} 
...
\vdots\\
\end{align*}
Similarly, we can dedcue the solution of the system
